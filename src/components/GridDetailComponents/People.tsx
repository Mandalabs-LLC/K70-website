        ...
        {peopleData.map((data, index)=>(
          <div key={index} className={`flex flex-col justify-center items-center bg-white gap-4 p-4 md:gap-20 md:p-20  ${index % 2 === 0 ? 'md:flex-row bg-opacity-90': 'md:flex-row-reverse opacity-1'}`}>
              ...
              <div className=" w-full h-full md:w-1/3 flex flex-col items-center">
                <Image src={data.image} alt="images" className={`shadow-md border-8 border-white ${index===1 && 'w-[280px]'}  `}/>
              {data.imageDescription &&  
               <p className='bg-white shadow-md text-blue-light text-xs tracking-wider text-center px-4 pb-2 w-[280px]'>{data.imageDescription}</p>}
              </div>
          </div>
        ))}
